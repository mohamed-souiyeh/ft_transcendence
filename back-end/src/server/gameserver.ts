...
export class gameServer implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.server.on('connection', (socket) => {
      if (this.clientCount == 0) room_.firstClient = socket;
      if (this.clientCount == 1) room_.secondClient = socket;
      this.clientCount++;
      this.clientCount %= 2;
    ...
    setInterval(() => {
      ...
      for (var i: number = 0; i < this.roomsList.length; i++) {
        if (
          this.roomsList[i].firstPaddlePos +
            this.roomsList[i].firstPaddleSpeed *
              this.roomsList[i].firstvelocity <
            7 / 10 - 1 / 10 &&
          this.roomsList[i].firstPaddlePos +
            this.roomsList[i].firstPaddleSpeed *
              this.roomsList[i].firstvelocity >
          ...
          this.roomsList[i].firstPaddlePos +=
            ...
            this.roomsList[i].firstvelocity;
        if (
          this.roomsList[i].secondPaddlePos +
            this.roomsList[i].secondPaddleSpeed *
              this.roomsList[i].secondvelocity <
            7 / 10 - 1 / 10 &&
          this.roomsList[i].secondPaddlePos +
            this.roomsList[i].secondPaddleSpeed *
              this.roomsList[i].secondvelocity >
        ...
        if (!this.roomsList[i].ballLaunched) {
          ...
          if (
            this.roomsList[i].ballPosX + 0.2 / 10 >= 0.97 &&
            this.roomsList[i].ballPosY - 0.2 / 10 <=
              this.roomsList[i].firstPaddlePos + 1 / 10 &&
            this.roomsList[i].ballPosY + 0.2 / 10 >=
              this.roomsList[i].firstPaddlePos - 1 / 10
          ) {
          ...
          if (
            this.roomsList[i].ballPosX - 0.2 / 10 <= -0.97 &&
            this.roomsList[i].ballPosY - 0.2 / 10 <=
              this.roomsList[i].secondPaddlePos + 1 / 10 &&
            this.roomsList[i].ballPosY + 0.2 / 10 >=
              this.roomsList[i].secondPaddlePos - 1 / 10
          ) {
            console.log('collisiooooon');
        ...
        this.roomsList[i].ballPosX +=
          this.roomsList[i].speed * this.roomsList[i].ballVelocityX;
        this.roomsList[i].ballPosY +=
          this.roomsList[i].speed * this.roomsList[i].ballVelocityY;
        ...
        this.server
          .to('room' + i)
          .emit('score', this.roomsList[i].score1, this.roomsList[i].score2);
        this.server
          .to('room' + i)
          .emit('left', this.roomsList[i].firstPaddlePos);
        this.server
          .to('room' + i)
          .emit('right', this.roomsList[i].secondPaddlePos);
